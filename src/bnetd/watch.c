...
static int handle_event_whisper(t_account *account, char const *gamename, t_clienttag clienttag, t_watch_event event)
{
    ...
    {
	if (gamename)
		sprintf(msg,"joingame:%s:%s:%s",myusername,game_title,gamename);
	else
		sprintf(msg,"joingame:%s:%s",myusername,game_title);
    }

	if (event == watch_event_leavegame)sprintf(msg,"leavegame:%s:%s:%s",myusername,game_title,gamename);
	if (event == watch_event_login)    sprintf(msg,"login:%s",myusername);
	if (event == watch_event_logout)   sprintf(msg,"logout:%s",myusername);
