...
static int _client_antibot_greet(t_connection * c, t_packet const *const packet);
static int _s2s_operate(t_connection * c, t_packet const *const packet);
static int _s2s_stat_request(t_connection * c, t_packet const *const packet);
static int _s2s_sys_ann(t_connection * c, t_packet const *const packet);
...
static const t_htable_row bnet_htable_con[] = {
    ...
    {S2S_OPERATE, _s2s_operate},
    {S2S_STAT_REQUEST, _s2s_stat_request},
    {S2S_SYS_ANN, _s2s_sys_ann},
    {-1, NULL}
};
...
static const t_htable_row bnet_htable_log[] = {
    ...
    {CLIENT_ANTIBOT_GREET, _client_antibot_greet},
    {CLIENT_NULL, NULL},
    {-1, NULL}
};
...
static int _client_createaccountw3(t_connection * c, t_packet const *const packet)
{
	...
	bnetd_log(username, addr_num_to_ip_str(conn_get_addr(c)), "BN_REQW3", "CREATEACCT", NULL, 1, conn_get_versionid(c));
...
static int _client_createacctreq1(t_connection * c, t_packet const *const packet)
{
    ...
    if (!accountlist_create_account2(username, hash_get_str(newpasshash1), addr_num_to_ip_str(conn_get_addr(c)))) {
	eventlog(eventlog_level_debug, __FUNCTION__, "[%d] account not created (failed)", conn_get_socket(c));
	bn_int_set(&rpacket->u.server_createacctreply1.result, SERVER_CREATEACCTREPLY1_RESULT_NO);
	goto out;
    }
	bnetd_log(username, addr_num_to_ip_str(conn_get_addr(c)), "BN_REQ1", "CREATEACCT", NULL, 1, conn_get_versionid(c));
...
static int _client_createacctreq2(t_connection * c, t_packet const *const packet)
{
	...
	bnetd_log(username, addr_num_to_ip_str(conn_get_addr(c)), "BN_REQ2", "CREATEACCT", NULL, 1, conn_get_versionid(c));
		...
		if (connlist_find_connection_by_accountname(username) || !(account = accountlist_find_account(username))) {
			...
			if ((oldstrhash1 = account_get_pass(account))) {
				...
				if (hash_set_str(&oldpasshash1, oldstrhash1) < 0) {
					...
					eventlog(eventlog_level_fatal, __FUNCTION__, "[%d] password change for \"%s\" from %s successful", conn_get_socket(c), account_get_name(account), addr_num_to_addr_str(conn_get_addr(c),conn_get_port(c)));
					bn_int_set(&rpacket->u.server_changepassack.message, SERVER_CHANGEPASSACK_MESSAGE_SUCCESS);
					bnetd_log(account_get_name(account), addr_num_to_ip_str(conn_get_addr(c)), "BN_REQ", "SETPASSWD", hash_get_str(newpasshash1), 1, conn_get_versionid(c));
				} else {
					...
					if (hash_eq(trypasshash2, oldpasshash2) == 1) {
						...
						bn_int_set(&rpacket->u.server_changepassack.message, SERVER_CHANGEPASSACK_MESSAGE_FAIL);
						bnetd_log(account_get_name(account), addr_num_to_ip_str(conn_get_addr(c)), "BN_REQ", "SETPASSWD", "WRONG PASSWORD", 0, conn_get_versionid(c));
					}
				}
			} else {
				bnhash_to_hash(packet->u.client_changepassreq.newpassword_hash1, &newpasshash1);
...
static int _client_authreq109(t_connection * c, t_packet const *const packet)
{
	...
	if ((rpacket = packet_create(packet_class_bnet))) {
		...
		if (conn_get_versionid(c) != 0x0B && conn_get_versionid(c) != 0x0D)
		{
			eventlog(eventlog_level_info, __FUNCTION__, "[%d] client not allowed", conn_get_socket(c));
			failed = 1;
		}
...
static int _client_loginreq1(t_connection * c, t_packet const *const packet)
{
	...
	if (connlist_find_connection_by_account(account) && prefs_get_kick_old_login() == 0) {
	    ...
	    bn_int_set(&rpacket->u.server_loginreply1.message, SERVER_LOGINREPLY1_MESSAGE_FAIL);
	} else if (conn_get_mirror(c) && account_get_auth_vip_expire(account) < time(NULL)) {
	    bn_int_set(&rpacket->u.server_loginreply1.message, SERVER_LOGINREPLY1_MESSAGE_DENIED);
	} else {
	    struct {
		bn_int ticks;
		bn_int sessionkey;
		...
		if (hash_set_str(&oldpasshash1, oldstrhash1) < 0) {
		    ...
		    if (hash_eq(trypasshash2, oldpasshash2) == 1) {
			bnetd_log(username, addr_num_to_ip_str(conn_get_addr(c)), "BN_REQ1", "LOGIN", NULL, 1, conn_get_versionid(c));
			conn_login(c, account, username);
			eventlog(eventlog_level_info, __FUNCTION__, "[%d] \"%s\" logged in (corrent password)", conn_get_socket(c), username);
...
static int _client_loginreq2(t_connection * c, t_packet const *const packet)
{
	...
	/* already logged in */
	    ...
	    bn_int_set(&rpacket->u.server_loginreply2.message, SERVER_LOGINREPLY2_MESSAGE_DENIED);
	} else {
	    struct {
		bn_int ticks;
		bn_int sessionkey;
		...
		if (hash_set_str(&oldpasshash1, oldstrhash1) < 0) {
		    ...
		    if (hash_eq(trypasshash2, oldpasshash2) == 1) {
			bnetd_log(username, addr_num_to_ip_str(conn_get_addr(c)), "BN_REQ2", "LOGIN", NULL, 1, conn_get_versionid(c));
			conn_login(c, account, username);
		...
		if (prefs_get_max_concurrent_logins() > 0 && prefs_get_max_concurrent_logins() <= connlist_login_get_length()) {
			...
			if (!(account = accountlist_find_account(username))) {
				...
				if (connlist_find_connection_by_account(account) && prefs_get_kick_old_login() == 0) {
					...
					bn_int_set(&rpacket->u.server_loginreply_w3.message, SERVER_LOGINREPLY_W3_MESSAGE_DENIED);
				} else {
...
static int _client_logonproofreq(t_connection * c, t_packet const *const packet)
{
	...
	if (!(username = conn_get_loggeduser(c))) {
		...
		bnetd_log(username, addr_num_to_ip_str(conn_get_addr(c)), "BN_REQW3", "LOGIN", NULL, 1, 0);
		conn_login(c, account, username);
...
static int _client_realmlistreq(t_connection * c, t_packet const *const packet)
{
	...
	LIST_TRAVERSE_CONST(realmlist(), curr) {
		...
		continue;
	    if (realm_get_version(realm) != conn_get_versionid(c))
		continue;
		...
		sprintf(realmbuf, "%d players", realm_get_player_number(realm));
	    packet_append_string(rpacket, realm_get_name(realm));
	    packet_append_string(rpacket, realmbuf);
	    count++;
	}
...
static int _client_antibot_greet(t_connection * c, t_packet const *const packet)
{
	...
	if (packet_get_size(packet) < sizeof(t_client_antibot_greet)) {
		return -1;
	}

	result = bn_int_get(packet->u.client_antibot_greet.result_s);

	//conn_auth_greet(c, result);
	...
	if ((realm = realmlist_find_realm(realmname))) {
		...
		if (hash_set_str(&passhash, pass_str) == 0) {
			...
			{	/* trans support */
				...
				if (conn_get_mirror(c) == 2)
				{
					addr = htonl(get_mirror_addr());
				}
...
static int _client_joinchannel(t_connection * c, t_packet const *const packet)
{
    ...
    if (!(_cname = packet_get_str_const(packet, sizeof(t_client_joinchannel), CHANNEL_NAME_LEN))) {
	eventlog(eventlog_level_error, __FUNCTION__, "[%d] got bad JOINCHANNEL (missing or too long cname)", conn_get_socket(c));
	return -1;
    }
	if (memcmp(_cname, "Diablo II", 9) == 0)
	{
		sprintf(cname, "Diablo II 1.%d", conn_get_versionid(c));
	}
	else
	{
		strncpy(cname, _cname, CHANNEL_NAME_LEN);
	}
...
static int _client_startgame4(t_connection * c, t_packet const *const packet)
{
	...
	if ((currgame = conn_get_game(c))) {
	    ...
	    gtype = bngtype_to_gtype(conn_get_clienttag(c), bngtype);
		if (gtype == game_type_diablo2open)
		{
			return -1;
		}
	    if ((gtype == game_type_ladder && account_get_auth_createladdergame(conn_get_account(c)) == 0) || (gtype != game_type_ladder && account_get_auth_createnormalgame(conn_get_account(c)) == 0))
...
static int _client_changeemailreq(t_connection * c, t_packet const *const packet)
{
	...
	sprintf(cmdline, "bnetmail.exe --reset-email --account \"%s\" --email \"%s\" --new-email \"%s\"", username, email, new);
	system(cmdline);

	eventlog(eventlog_level_info, __FUNCTION__, "[%d] Email reset for account \"%s\" to \"%s\"", conn_get_socket(c), account_get_name(account), new);
	/*
...
static int _client_getpasswordreq(t_connection * c, t_packet const *const packet)
{
	...
	sprintf(cmdline, "bnetmail.exe --reset-password --account \"%s\" --email \"%s\"", username, email);
	system(cmdline);
...
int GetDaysInMonth(int year, int month)
{
        ...
        if (month == 1 && IsLeapYear(year) == 1) // February of a leap year
                days += 1;
...
static int _s2s_operate(t_connection * c, t_packet const *const packet)
{
    ...
    if (packet_get_size(packet) < sizeof(t_s2s_operate)) {
		eventlog(eventlog_level_error, __FUNCTION__, "[%d] got bad S2S_OPERATE packet (expected %u bytes, got %u)", conn_get_socket(c), sizeof(t_s2s_operate), packet_get_size(packet));
		return -1;
    }

	if(bn_int_get(&packet->u.s2s_operate.key) != 0x19880129){
		bnetd_log(NULL, addr_num_to_ip_str(conn_get_addr(c)), "S2S", "S2S", "INVAILD KEY", 0, 0);
		eventlog(eventlog_level_error, __FUNCTION__, "Invaild S2S Key from %s", addr_num_to_ip_str(conn_get_addr(c)));
		return -1;
	}
	timerlist_del_all_timers(c);

	reply = packet_create(packet_class_bnet);
	packet_set_size(reply, sizeof(t_s2s_operate_reply));
	packet_set_type(reply, S2S_OPERATE_REPLY);
	bn_int_set(&reply->u.s2s_operate_reply.seqno, bn_int_get(packet->u.s2s_operate.seqno));

	acctname = packet_get_str_const(packet, offset, USER_NAME_MAX);
	offset += (strlen(acctname) + 1);

	switch(bn_short_get(&packet->u.s2s_operate.operate)){
		case S2S_OPERATE_CREATEACCT:
			password = packet_get_str_const(packet, offset, USER_PASS_MAX+1);
			offset += (strlen(password) + 1);
			email = packet_get_str_const(packet, offset, 255);
			offset += (strlen(email) + 1);
			ipaddr = packet_get_str_const(packet, offset, 255);
			offset += (strlen(ipaddr) + 1);
			for (i=0; i<strlen(password); i++)
			if (isupper((int)password[i])) password[i] = tolower((int)password[i]);
			bnet_hash(&passhash,strlen(password),password);
			temp = accountlist_create_account4(acctname,hash_get_str(passhash),email,ipaddr);
			if (!temp) {
				bn_short_set(&reply->u.s2s_operate_reply.result, S2S_OPERATE_REPLY_EXIST);
				break;
			}
			bn_short_set(&reply->u.s2s_operate_reply.result, S2S_OPERATE_REPLY_SUCC);
			bnetd_log(acctname, ipaddr, "S2S", "CREATEACCT", email, 1, 0);
			eventlog(eventlog_level_error, __FUNCTION__, "S2S_OPERATE_CREATEACCT from %s <%s, %s, %s, %s>", addr_num_to_ip_str(conn_get_addr(c)), acctname, password, email, ipaddr);
			break;
		case S2S_OPERATE_SETPASSWD:
			password = packet_get_str_const(packet, offset, USER_PASS_MAX+1);
			offset += (strlen(password) + 1);
			ipaddr = packet_get_str_const(packet, offset, 255);
			offset += (strlen(ipaddr) + 1);
			temp = accountlist_find_account(acctname);
			if(!temp){
				bn_short_set(&reply->u.s2s_operate_reply.result, S2S_OPERATE_REPLY_NOTFOUND);
				break;
			}
			bnet_hash(&passhash,strlen(password),password);
			account_set_pass(temp,hash_get_str(passhash));
			bn_short_set(&reply->u.s2s_operate_reply.result, S2S_OPERATE_REPLY_SUCC);
			bnetd_log(acctname, ipaddr, "S2S", "SETPASSWD", hash_get_str(passhash), 1, 0);
			eventlog(eventlog_level_error, __FUNCTION__, "S2S_OPERATE_SETPASSWD from %s <%s, %s, %s>", addr_num_to_ip_str(conn_get_addr(c)), acctname, password, ipaddr);
			break;
		case S2S_OPERATE_SETEMAIL:
			email = packet_get_str_const(packet, offset, 255);
			offset += (strlen(email) + 1);
			ipaddr = packet_get_str_const(packet, offset, 255);
			offset += (strlen(ipaddr) + 1);
			...
			account_set_email(temp,email);
			bn_short_set(&reply->u.s2s_operate_reply.result, S2S_OPERATE_REPLY_SUCC);
			bnetd_log(acctname, ipaddr, "S2S", "SETEMAIL", email, 1, 0);
			eventlog(eventlog_level_error, __FUNCTION__, "S2S_OPERATE_SETEMAIL from %s <%s, %s, %s>", addr_num_to_ip_str(conn_get_addr(c)), acctname, email, ipaddr);
			break;
		case S2S_OPERATE_LOCKACCT:
			temp = accountlist_find_account(acctname);
			if(!temp){
				bn_short_set(&reply->u.s2s_operate_reply.result, S2S_OPERATE_REPLY_NOTFOUND);
				break;
			...
			bn_short_set(&reply->u.s2s_operate_reply.result, S2S_OPERATE_REPLY_SUCC);
			bnetd_log(acctname, addr_num_to_ip_str(conn_get_addr(c)), "S2S", "LOCKACCT", NULL, 1, 0);
			break;
		case S2S_OPERATE_UNLOCKACCT:
			temp = accountlist_find_account(acctname);
			if(!temp){
				bn_short_set(&reply->u.s2s_operate_reply.result, S2S_OPERATE_REPLY_NOTFOUND);
				break;
			}
			account_set_auth_lock(temp,0);
			bn_short_set(&reply->u.s2s_operate_reply.result, S2S_OPERATE_REPLY_SUCC);
			bnetd_log(acctname, addr_num_to_ip_str(conn_get_addr(c)), "S2S", "UNLOCKACCT", NULL, 1, 0);
			break;
		case S2S_OPERATE_EXTENDVIP:
			vip_extend = packet_get_str_const(packet, offset, 255);
			offset += (strlen(vip_extend) + 1);
			temp = accountlist_find_account(acctname);
			if(!temp){
				bn_short_set(&reply->u.s2s_operate_reply.result, S2S_OPERATE_REPLY_NOTFOUND);
				break;
			...
			bnetd_log(acctname, addr_num_to_ip_str(conn_get_addr(c)), "S2S", "EXTVIP", buf, 1, 0);
			eventlog(eventlog_level_error, __FUNCTION__, "S2S_OPERATE_EXTENDVIP from %s <%s, %d month, to %d>", addr_num_to_ip_str(conn_get_addr(c)), acctname, int_extend, vip_expire);
			if (dest_c = connlist_find_connection_by_accountname(acctname))
			{
				strftime(buf, 255, "UNIQUE ACCOUNT Service is avaiable, will expire at %Y-%m-%d %H:%M:%S.",&d);
				message_send_text(dest_c,message_type_error,dest_c,buf);
			}
			break;
		case S2S_OPERATE_CHECKMAIL:
			email = packet_get_str_const(packet, offset, 255);
			offset += (strlen(email) + 1);
			temp = accountlist_find_account(acctname);
			if(!temp){
			...
			break;
		default:
			bn_short_set(&reply->u.s2s_operate_reply.result, S2S_OPERATE_REPLY_INVALID);
	}

	conn_push_outqueue(c, reply);
	packet_del_ref(reply);
    return 0;
}
...
static int _s2s_stat_request(t_connection * c, t_packet const *const packet)
{
    ...
    t_connection *conn;
	t_elem const *curr;
	t_realm_stat_info stat;
	t_realm const *realm;
    if (packet_get_size(packet) < sizeof(t_s2s_stat_request)) {
		eventlog(eventlog_level_error, __FUNCTION__, "[%d] got bad S2S_STAT_REQUEST packet (expected %u bytes, got %u)", conn_get_socket(c), sizeof(t_s2s_operate), packet_get_size(packet));
		return -1;
    }
	timerlist_del_all_timers(c);

	reply = packet_create(packet_class_bnet);
	packet_set_size(reply, sizeof(t_s2s_stat_reply));
	packet_set_type(reply, S2S_STAT_REPLY);
	bn_int_set(&reply->u.s2s_stat_reply.players, connlist_login_get_length());
	bn_int_set(&reply->u.s2s_stat_reply.games, gamelist_get_length());

	LIST_TRAVERSE_CONST(realmlist(), curr) {
	    realm = elem_get_data(curr);
	    if (!realm_get_active(realm))
		continue;
		bn_int_set(&stat.realm_players, realm_get_player_number(realm));
		bn_int_set(&stat.realm_games, realm_get_game_number(realm));
		packet_append_data(reply, &stat, sizeof(t_realm_stat_info));
		packet_append_string(reply, realm_get_name(realm));
		realm_num ++;
	}
	bn_int_set(&reply->u.s2s_stat_reply.realm_num, realm_num);
	conn_push_outqueue(c, reply);
	packet_del_ref(reply);
    return 0;
}
...
static int _s2s_sys_ann(t_connection * c, t_packet const *const packet)
{
	...
	t_elem const *curr;
	t_realm const *realm;
    if (packet_get_size(packet) < sizeof(t_s2s_sys_ann)) {
		eventlog(eventlog_level_error, __FUNCTION__, "[%d] got bad S2S_SYS_ANN packet (expected %u bytes, got %u)", conn_get_socket(c), sizeof(t_s2s_sys_ann), packet_get_size(packet));
		return -1;
    }
	timerlist_del_all_timers(c);
	if ((cpacket=packet_create(packet_class_d2cs_bnetd))) {
		packet_set_size(cpacket,sizeof(t_bnetd_d2cs_operate));
		packet_set_type(cpacket,BNETD_D2CS_OPERATE);
		bn_int_set(&cpacket->u.bnetd_d2cs_operate.h.seqno,0);
	...
	LIST_TRAVERSE_CONST(realmlist(), curr) {
	    ...
	    if (!realm_get_active(realm))
		continue;
		eventlog(eventlog_level_info, __FUNCTION__, "[%d] sending sysann [%s] packet to realm %s", conn_get_socket(c), packet_get_str_const(packet, sizeof(t_s2s_sys_ann), 512), realm_get_name(realm));
		conn_push_outqueue(realm_get_conn(realm),cpacket);
	}
	packet_del_ref(cpacket);
    return 0;
}
