...
static int sql_bnetd_log(const char*, const char*, const char*, const char*, const char*, unsigned int);
...
static int sql_bnetd_log(const char* acctname, const char* ipaddr, const char* opt, const char* type, const char* addi, unsigned int ifsucc, unsigned int version){
	char buf[1024];
	char fmtmsg[255];
	int stlen;
	char* pos;
	...
	sprintf(buf, "INSERT DELAYED INTO bnetdlog_buffer (`acctname`, `ipaddr`, `operator`, `type`, `optime`, `addi`, `ifsucc`, `version`) VALUES ('%s', '%s', '%s', '%s', UNIX_TIMESTAMP(), '%s', '%d', '%d')", acctname, ipaddr, opt, type, fmtmsg, ifsucc, version);
	sql->query(buf);
	return 0;
}
...
static t_attr *sql_read_attr(t_storage_info * info, const char *key)
{
    ...
    snprintf(query, sizeof(query), "SELECT %c%s%c FROM %s%s WHERE " SQL_UID_FIELD " = %u", sql->quote, col, sql->quote, tab_prefix, tab, uid);
    if ((result = sql->query_res(query)) == NULL)
	return NULL;
...
int sql_write_attrs(t_storage_info * info, const t_hlist *attrs)
{
	...
	snprintf(query, sizeof(query), "UPDATE %s%s SET %c%s%c = '%s' WHERE "SQL_UID_FIELD" = '%u'", tab_prefix, tab, quote, col, quote, escape, uid);
...
//	    eventlog(eventlog_level_debug, __FUNCTION__, "trying to insert new column %s", col);
	    snprintf(query2, sizeof(query2), "ALTER TABLE %s%s ADD COLUMN %c%s%c VARCHAR(128)", tab_prefix, tab, quote, col, quote);
...
//          eventlog(eventlog_level_trace, "db_set", "retry insert query: %s", query);
		...
		snprintf(query2, sizeof(query2), "INSERT INTO %s%s ("SQL_UID_FIELD",%c%s%c) VALUES ('%u','%s')", tab_prefix, tab, quote, col, quote, uid, escape);
