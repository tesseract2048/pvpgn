...
static unsigned long mirror_addr = 0;
...
static int sd_accept(t_addr const * curr_laddr, t_laddr_info const * laddr_info, int ssocket, int usocket)
{
...
#ifdef HAVE_IOCP
    if ((csocket = iocp_accept((struct sockaddr *)&caddr,&caddr_len))<0)
#else
	...
	if (caddr.sin_addr.s_addr != mirror_addr)
	{
	...
	eventlog(eventlog_level_error,__FUNCTION__,"s_addr: %x, mirror_addr: %x", caddr.sin_addr.s_addr, mirror_addr);
	if (!(c = conn_create(csocket,usocket,raddr,rport,addr_get_ip(curr_laddr),addr_get_port(curr_laddr),ntohl(caddr.sin_addr.s_addr),ntohs(caddr.sin_port),(caddr.sin_addr.s_addr == mirror_addr))))
	{
...
static int sd_tcpinput(t_connection * c)
{
	...
	if (!conn_get_in_queue(c))
	{
		...
		case conn_class_init:
			if (conn_get_mirror(c) == 1)
			{
				if (!(packet = packet_create(packet_mirror_greet)))
				{
		...
		case conn_class_d2cs_bnetd:
			eventlog(eventlog_level_fatal,__FUNCTION__,"d2cs sock: %d", csocket);
	...
	switch (net_recv_packet(conn_get_fdw_idx(c), csocket,packet,&currsize))
	{
	case -1:
...
static int sd_tcpoutput(t_connection * c)
{
	...
	switch (net_send_packet(conn_get_fdw_idx(c), csocket,packet,&currsize)) /* avoid warning */
	{
	case -1:
		/* marking connection as "destroyed", memory will be freed later */
	    conn_set_state(c, conn_state_destroy);
...
static int _setup_listensock(t_addrlist *laddrs)
{
	...
	if (!addr_get_addr_str(curr_laddr,tempa,sizeof(tempa)))
	    strcpy(tempa,"x.x.x.x:x");
#ifdef HAVE_IOCP
	laddr_info->ssocket = WSASocket(AF_INET,SOCK_STREAM, 0, NULL, 0, WSA_FLAG_OVERLAPPED);
#else
	...
	fidx = fdwatch_add_fd(laddr_info->ssocket, fdwatch_type_read | fdwatch_type_accept, handle_accept, curr_laddr);
	if (fidx < 0) {
...
extern int server_process(void)
{
	...
	if (prefs_get_mirror_ip())
	{
		mirror_addr = inet_addr(prefs_get_mirror_ip());
	}
...
extern unsigned long get_mirror_addr(void){
	return mirror_addr;
}