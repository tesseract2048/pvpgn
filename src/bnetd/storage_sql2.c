...
static int sql2_bnetd_log(const char*, const char*, const char*, const char*, const char*, unsigned int);
...
static int _db_get_tab(const char *key, char **ptab, char **pcol)
{
    ...
    *p = 0;
    ...
    strncpy(col, key + strlen(tab) + 1, DB_MAX_ATTRKEY - 1);
...
static int sql2_bnetd_log(const char* acctname, const char* ipaddr, const char* opt, const char* type, const char* addi, unsigned int ifsucc){
	char buf[1024];
	sprintf(buf, "INSERT DELAYED INTO bnetdlog_buffer (acctname, ipaddr, operator, type, optime, addi, ifsucc) VALUES ('%s', '%s', '%s', '%s', UNIX_TIMESTAMP(), '%s', '%d')", acctname, ipaddr, opt, type, addi, ifsucc);
	sql->query(buf);
	return 0;
}
...
static t_attr *sql2_read_attr(t_storage_info * info, const char *key)
{
    ...
    sql->escape_string(esckey, col, strlen(col));
...
static int sql2_write_attrs(t_storage_info * info, const t_hlist *attrs)
{
	...
	sql->escape_string(escval, safeval, strlen(safeval));
