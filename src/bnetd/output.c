...
extern void output_init(void)
{
	...
	status_filename = create_filename(prefs_get_outputdir(),"status",".html"); // WarCraft III
...
int output_standard_writer(FILE * fp)
{
    ...
    {
	fprintf(fp,"%d\n%d\n%d",gamelist_get_length(),connlist_login_get_length(),accountlist_get_length());
	/*fprintf(fp,"[CHANNELS]\n");
	number=1;
	LIST_TRAVERSE_CONST(channellist(),curr)
	{
