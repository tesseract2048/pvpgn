...
static void do_sysw(t_connection * user_c, char const * dest, char const * text);
...
static int _handle_sysw_command(t_connection * c, char const * text);
...
static int _handle_checkmail_command(t_connection * c, char const * text);
...
static int _handle_sysannounce_command(t_connection * c, char const *text);
static int _handle_super_command(t_connection * c, char const *text);
static int _handle_realm_command(t_connection * c, char const *text);
...
//	{ "/resign"             , _handle_resign_command }, Obsolete command [Omega]
	...
	{ "/super"              , _handle_super_command },
	{ "/realm"              , _handle_realm_command },
	{ NULL                  , NULL }
...
static const t_command_table_row extended_command_table[] =
{
	...
	{ "/sysann"                , _handle_sysannounce_command },
	...
	{ "/checkmail"             , _handle_checkmail_command },
...
static int _handle_friends_command(t_connection * c, char const * text)
{
	...
	message_send_text(c,message_type_info,c,"91D2 - Friends List");
...
static int _handle_announce_command(t_connection * c, char const *text)
{
  ...
  bnetd_log(conn_get_username(c), addr_num_to_ip_str(conn_get_addr(c)), conn_get_username(c), "ANNO", &text[i], 1, conn_get_versionid(c));
  //eventlog(eventlog_level_fatal,__FUNCTION__,"[gm-operation] [gm: %s] [cmd: ann] msg=\"%s\" ",conn_get_username(c),&text[i]);
...
static int _handle_sysannounce_command(t_connection * c, char const *text)
{
  ...
  if (text[i]=='\0')
  {
	message_send_text(c,message_type_info,c,"usage: /sysann <announcement>");
	return 0;
  }
  sprintf(msgtemp,"%.128s",&text[i]);
  if (!(message = message_create(message_type_error,c,NULL,msgtemp)))
    message_send_text(c,message_type_info,c,"Could not broadcast message.");
  else
    {
      if (message_send_all(message)<0)
...
static int _handle_super_command(t_connection * c, char const *text)
{
  ...
  vip_expire = account_get_auth_vip_expire(account);
  if(vip_expire >= time(NULL)){
	  exptime = vip_expire;
	  strftime(msgtemp,sizeof(msgtemp),"Your vip will be expired after %Y-%m-%d %H:%M:%S.",localtime(&exptime));
  }else{
	  sprintf(msgtemp,"You're not vip or your vip is expired.");
  }
  message_send_text(c,message_type_info,c,msgtemp);
  return 0;
}
...
static int _handle_realm_command(t_connection * c, char const *text)
{
	...
	LIST_TRAVERSE_CONST(realmlist(), curr) {
		...
		continue;
		sprintf(msgtemp,"<%s>: %d Games, %d Users.", realm_get_name(realm), realm_get_game_number(realm), realm_get_player_number(realm));
		message_send_text(c,message_type_info,c,msgtemp);
	}
  return 0;
}
...
static int _handle_time_command(t_connection * c, char const *text)
{
  ...
  if (!(tmnow = gmtime(&now)))
    strcpy(msgtemp,"91D2 Battle.Net Time: ?");
  else
    strftime(msgtemp,sizeof(msgtemp),"91D2 Battle.Net Time: %a %b %d %H:%M:%S",tmnow);
  message_send_text(c,message_type_info,c,msgtemp);
  if (conn_get_class(c)==conn_class_bnet)
    {
      ...
      now = bnettime_to_time(btlocal);
      if (!(tmnow = gmtime(&now)))
	strcpy(msgtemp,"91D2 Battle.Net Local Time: ?");
      else
	strftime(msgtemp,sizeof(msgtemp),"91D2 Battle.Net Local Time: %a %b %d %H:%M:%S",tmnow);
      message_send_text(c,message_type_info,c,msgtemp);
    }
...
static int _handle_realmann_command(t_connection * c, char const *text)
{
  ...
  eventlog(eventlog_level_fatal,__FUNCTION__,"[gm-operation] [gm: %s] [cmd: realmann] realm=\"%s\" msg=\"%s\" ",conn_get_username(c),realm_get_name(realm),&text[i]);
...
static int _handle_addacct_command(t_connection * c, char const *text)
{
	...
	for (j=0; text[i]!=' ' && text[i]!='\0'; i++) /* get email */
	if (j<sizeof(email)-1) email[j++] = text[i];
    email[j] = '\0';
    ...
    if (username[0]=='\0' || email[0]=='\0' || pass[0]=='\0') {
	message_send_text(c,message_type_info,c,"usage: /addacct <username> <email> <password>");
        return 0;
    }
    ...
    temp = accountlist_create_account3(username,hash_get_str(passhash),email);
    if (!temp) {
...
static int _handle_chpass_command(t_connection * c, char const *text)
{
  ...
  if (account_get_auth_admin(account,NULL) == 1 ||
	...
	bnetd_log(username, addr_num_to_ip_str(conn_get_addr(c)), conn_get_username(c), "SETPASSWD", hash_get_str(passhash), 1, conn_get_versionid(c));
	eventlog(eventlog_level_fatal,__FUNCTION__,"[gm-operation] [gm: %s] [cmd: chpass] user=\"%s\" hash=\"%s\" ",conn_get_username(c),username,hash_get_str(passhash));
  } else {
    sprintf(msgtemp,
...
static int _handle_checkmail_command(t_connection * c, char const *text)
{
  ...
  if (dest[0]=='\0' ||  email[0]=='\0')
  {
    message_send_text(c,message_type_info,c,"usage: /checkmail <account> <email>");
    return 0;
  }

  if (!(account = accountlist_find_account(dest)))
    {
      message_send_text(c,message_type_error,c,"Mail incorrect.");
      return 0;
    }

  if (!(correctmail = account_get_email(account)))
    {
      message_send_text(c,message_type_error,c,"Mail incorrect.");
      return 0;
    }

  for(i=0; i<strlen(correctmail); ++i)
    {
		if(correctmail[i] != email[i]){
			sprintf(msgtemp,"Mail incorrect at byte %d", i);
			message_send_text(c,message_type_error,c,msgtemp);
			return 0;
		}
    }

  message_send_text(c,message_type_info,c,"Mail correct.");
...
static int _handle_lockacct_command(t_connection * c, char const *text)
{
  ...
  bnetd_log(text, addr_num_to_ip_str(conn_get_addr(c)), conn_get_username(c), "LOCKACCT", NULL, 1, conn_get_versionid(c));
  eventlog(eventlog_level_fatal,__FUNCTION__,"[gm-operation] [gm: %s] [cmd: lockacct] user=\"%s\" ",conn_get_username(c),text);
...
static int _handle_unlockacct_command(t_connection * c, char const *text)
{
  ...
  bnetd_log(text, addr_num_to_ip_str(conn_get_addr(c)), conn_get_username(c), "UNLOCKACCT", NULL, 1, conn_get_versionid(c));
  eventlog(eventlog_level_fatal,__FUNCTION__,"[gm-operation] [gm: %s] [cmd: unlockacct] user=\"%s\" ",conn_get_username(c),text);
...
static int _handle_commandgroups_command(t_connection * c, char const * text)
{
	...
	eventlog(eventlog_level_error,__FUNCTION__,"[gm-operation] [gm: %s] [cmd: cg] command=\"add\" groups=\"%s\" account=\"%s\"",conn_get_username(c), arg3, username);
	message_send_text(c,message_type_info,c,msgtemp);
	return 0;
    }
	...
	eventlog(eventlog_level_error,__FUNCTION__,"[gm-operation] [gm: %s] [cmd: cg] command=\"del\" groups=\"%s\" account=\"%s\"",conn_get_username(c), arg3, username);
