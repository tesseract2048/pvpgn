...
static int on_d2cs_joingamesignal(t_connection * c, t_packet const * packet);
static int on_d2cs_charoptsignal(t_connection * c, t_packet const * packet);
		...
		case conn_state_loggedin:
			switch (packet_get_type(packet)) {
				...
				case D2CS_BNETD_GAMEINFOREPLY:
					...
					break;
				case D2CS_BNETD_JOINGAMESIGNAL:
					on_d2cs_joingamesignal(c,packet);
					break;
				case D2CS_BNETD_CHAROPTSIGNAL:
					on_d2cs_charoptsignal(c,packet);
					break;
				default:
					eventlog(eventlog_level_error,__FUNCTION__,
					"got unknown packet type %d",packet_get_type(packet));
...
static int on_d2cs_authreply(t_connection * c, t_packet const * packet)
{
	...
	if ((rpacket=packet_create(packet_class_d2cs_bnetd))) {
		...
		bn_byte_set(&rpacket->u.bnetd_d2cs_authreply.vip_realm,realm_get_vip_realm(realm));
...
static int on_d2cs_accountloginreq(t_connection * c, t_packet const * packet)
{
	...
	if ((rpacket=packet_create(packet_class_d2cs_bnetd))) {
		...
		bn_int_set(&rpacket->u.bnetd_d2cs_accountloginreply.vip_expire,account_get_auth_vip_expire(conn_get_account(client)));
		bn_int_set(&rpacket->u.bnetd_d2cs_accountloginreply.addr,conn_get_addr(client));
		conn_push_outqueue(c,rpacket);
		packet_del_ref(rpacket);
	}
	return 0;
...
static int on_d2cs_joingamesignal(t_connection * c, t_packet const * packet)
{
	...
	t_packet *	rpacket;
	char buf[512];
	if (packet_get_size(packet)<sizeof(t_d2cs_bnetd_joingamesignal)) {
		eventlog(eventlog_level_error,__FUNCTION__,"got bad packet size");
		return -1;
	}
	pos=sizeof(t_d2cs_bnetd_joingamesignal);
	if (!(account=packet_get_str_const(packet,pos,USER_NAME_MAX))) {
		return -1;
	}
	pos+=strlen(account)+1;
	if (!(charname=packet_get_str_const(packet,pos,CHAR_NAME_LEN))) {
		return -1;
	}
	pos+=strlen(charname)+1;
	if (!(ipaddr=packet_get_str_const(packet,pos,16))) {
		return -1;
	}
	pos+=strlen(ipaddr)+1;
	if (!(gamename=packet_get_str_const(packet,pos,64))) {
		return -1;
	}
	pos+=strlen(gamename)+1;
	if (!(gamepass=packet_get_str_const(packet,pos,GAME_PASS_LEN))) {
		return -1;
	}
	sprintf(buf, "%s,%s,%s", charname, gamename, gamepass);
	bnetd_log(account, ipaddr, account, "JOINGAME", buf, 1, realm_get_version(conn_get_realm(c)));
	return 0;
}
...
static int on_d2cs_charoptsignal(t_connection * c, t_packet const * packet)
{
	...
	unsigned int	pos;
	t_packet *	rpacket;
	if (packet_get_size(packet)<sizeof(t_d2cs_bnetd_charoptsignal)) {
		eventlog(eventlog_level_error,__FUNCTION__,"got bad packet size");
		return -1;
	}
	pos=sizeof(t_d2cs_bnetd_charoptsignal);
	if (!(account=packet_get_str_const(packet,pos,USER_NAME_MAX))) {
		return -1;
	}
	pos+=strlen(account)+1;
	...
	pos+=strlen(ipaddr)+1;
	if(bn_int_get(&packet->u.d2cs_bnetd_charoptsignal.opt_type) == 1){
		bnetd_log(account, ipaddr, account, "DELCHAR", charname, 1, realm_get_version(conn_get_realm(c)));
	}
	if(bn_int_get(&packet->u.d2cs_bnetd_charoptsignal.opt_type) == 2){
		bnetd_log(account, ipaddr, account, "CONVCHAR", charname, 1, realm_get_version(conn_get_realm(c)));
	}
	if(bn_int_get(&packet->u.d2cs_bnetd_charoptsignal.opt_type) == 3){
		bnetd_log(account, ipaddr, account, "CREATECHAR", charname, 1, realm_get_version(conn_get_realm(c)));
	}
	return 0;
}