...
extern int antibot_hash(const int key_s, const int word_s){
	return ((key_s * 2 + 2056) ^ (word_s - 91));
}
...
//	temp->protocol.client.antibot_total			 = 0;
//	temp->protocol.client.antibot_correct		 = 0;
//	temp->protocol.client.antibot_key			 = 0;
//	temp->protocol.client.antibot_word			 = 0;
...
extern void conn_destroy(t_connection * c, t_elem ** elem, int conn_or_dead_list)
{
   ...
   if ((c->protocol.game) && (c->protocol.account))
   {
	...
	if (c->protocol.account)
	{
		...
		/*if (c->protocol.client.antibot_correct < c->protocol.client.antibot_total){
			eventlog(eventlog_level_fatal,__FUNCTION__,"*** AntiBot: \"%s\" has got a big problem (correct: %d, total: %d)",conn_get_loggeduser(c), c->protocol.client.antibot_correct, c->protocol.client.antibot_total);
		}*/
...
extern void conn_login(t_connection *c, t_account *a, const char *loggeduser)
{
	...
	eventlog(eventlog_level_fatal, __FUNCTION__, "[%d] \"%s\" from %s logged in (right password)", conn_get_socket(c), loggeduser, addr_num_to_ip_str(conn_get_addr(c)));
...
extern unsigned int conn_antibot_greet(t_connection * c)
{
	t_packet *rpacket;
	if (!(rpacket = packet_create(packet_class_bnet)))
	    return -1;
/*	c->protocol.client.antibot_key = rand();
	c->protocol.client.antibot_word = rand();
	c->protocol.client.antibot_total ++;
	packet_set_size(rpacket, sizeof(t_server_antibot_greet));
	packet_set_type(rpacket, SERVER_ANTIBOT_GREET);
	bn_int_set(&rpacket->u.server_antibot_greet.key_s, c->protocol.client.antibot_key);
	bn_int_set(&rpacket->u.server_antibot_greet.word_s, c->protocol.client.antibot_word);
	conn_push_outqueue(c, rpacket);
	packet_del_ref(rpacket);*/
	return 0;
}
...
extern unsigned int conn_auth_greet(t_connection * c, int result){
	...
	/*key_s = c->protocol.client.antibot_key;
	word_s = c->protocol.client.antibot_word;

	result_c = antibot_hash(key_s,word_s);

	if(result == result_c){
		c->protocol.client.antibot_correct ++;
		eventlog(eventlog_level_info, __FUNCTION__, "[%d] Correct ANTIBOT_GREET response, key_s = 0x%x, word_s = 0x%x, result = 0x%x", conn_get_socket(c), key_s, word_s, result, result_c);
	}else{
		eventlog(eventlog_level_fatal, __FUNCTION__, "[%d] BAD ANTIBOT_GREET response, key_s = 0x%x, word_s = 0x%x, result = 0x%x, correct = 0x%x", conn_get_socket(c), key_s, word_s, result, result_c);
	}*/
	return 0;
}
