...
extern int net_recv(int idx, int sock, void *buff, int len)
{
    int res;
...
extern int net_recv_packet(int idx, int sock, t_packet * packet, unsigned int * currsize)
{
	int          addlen;
	...
	if (*currsize<header_size)
		addlen = net_recv(idx, sock, temp, header_size-*currsize);
	else {
		...
		addlen = net_recv(idx, sock, temp, total_size-*currsize);
	}
...
extern int net_send(int idx, int sock, const void *buff, int len)
{
    int res;
...
extern int net_send_packet(int idx, int sock, t_packet const * packet, unsigned int * currsize)
{
    unsigned int size;
    int          addlen;
	...
	*currsize = 0;
    ...
    addlen = net_send(idx, sock,packet_get_raw_data_const(packet,*currsize),size-*currsize);
