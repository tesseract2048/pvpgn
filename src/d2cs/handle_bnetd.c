...
DECLARE_PACKET_HANDLER(on_bnetd_operate)
...
/* 0x13 */ { 0,                                      conn_state_none,      NULL                       },
/* 0x14 */ { 0,                                      conn_state_none,      NULL                       },
/* 0x15 */ { sizeof(t_bnetd_d2cs_operate),			 conn_state_authed,	   on_bnetd_operate		      }
};
...
static int on_bnetd_authreply(t_connection * c, t_packet * packet)
{
	...
	if (reply == BNETD_D2CS_AUTHREPLY_SUCCEED) {
		...
		vip_realm = bn_byte_get(packet->u.bnetd_d2cs_authreply.vip_realm);
	} else {
...
static int on_bnetd_accountloginreply(t_connection * c, t_packet * packet)
{
	...
	if (result==BNETD_D2CS_CHARLOGINREPLY_SUCCEED) {
		...
		d2cs_conn_set_addr(client, bn_int_get(packet->u.bnetd_d2cs_accountloginreply.addr));
		d2cs_conn_set_vip_expire(client, bn_int_get(packet->u.bnetd_d2cs_accountloginreply.vip_expire));
...
static int on_bnetd_operate(t_connection * c, t_packet * packet)
{
	t_packet *rpacket;
	if (!packet || !c)
	    return -1;
	
	eventlog(eventlog_level_info,__FUNCTION__,"sending sysann packet [%s]",packet_get_str_const(packet,sizeof(t_d2cs_d2gs_operate),512));
	if ((rpacket=packet_create(packet_class_d2gs))) {
		packet_set_size(rpacket,sizeof(t_d2cs_d2gs_operate));
        packet_set_type(rpacket,D2CS_D2GS_OPERATE);
		packet_append_string(rpacket, packet_get_str_const(packet,sizeof(t_d2cs_d2gs_operate),512));
		bn_byte_set(&rpacket->u.d2cs_d2gs_operate.opt, bn_byte_get(packet->u.bnetd_d2cs_operate.opt));
		d2gs_annpacket(rpacket);
		packet_del_ref(rpacket);
	}
	return 0;
}
