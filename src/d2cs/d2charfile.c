...
/*	Procedure is stopped here and returned if
	...
	if (bn_int_get(charinfo.summary.charlevel) < 8) {
		eventlog(eventlog_level_info,__FUNCTION__,"Convert char has been disabled for level limit");
		fclose(fp);
		return -1;
	}
	if (d2charinfo_get_difficulty(&charinfo.summary) < 3) {
		eventlog(eventlog_level_info,__FUNCTION__,"Convert char has been disabled for difficulty limit(current %d, need %d)", d2charinfo_get_difficulty(&charinfo.summary), 3);
		fclose(fp);
		return -1;
	}
	...
	/*if(bn_int_get((bn_int *)(buffer+D2CHARSAVE_CHECKSUM_OFFSET)) != d2charsave_checksum(buffer,size,D2CHARSAVE_CHECKSUM_OFFSET)){
		eventlog(eventlog_level_fatal,__FUNCTION__,"%s: invaild checksum occured while converting",charname);
		fclose(fp);
		return -1;
	}*/
...
extern int d2char_delete(char const * account, char const * charname)
{
	...
	/* bak charinfo file */	
	sprintf(filepath,"%s/%s",prefs_get_bak_charinfo_dir(),account);
	...
	deltime = time(NULL);
	sprintf(filepath,"%s/%s",prefs_get_bak_charsave_dir(),account);
	if (stat(filepath,&statbuf)==-1) {
		mkdir(filepath,S_IRWXU|S_IRWXG|S_IRWXO );
		eventlog(eventlog_level_info,__FUNCTION__,"created charsave directory: %s",filepath);
	}
	sprintf(bakfile,"%s/%s/%s+%d",prefs_get_bak_charinfo_dir(),account,charname,deltime);
	remove(bakfile);
	file=xmalloc(strlen(prefs_get_charinfo_dir())+1+strlen(account)+1+strlen(charname)+1);
	d2char_get_infofile_name(file,account,charname);
	rename(file,bakfile);
	...
	sprintf(bakfile,"%s/%s/%s+%d",prefs_get_bak_charsave_dir(),account,charname,deltime);
	remove(bakfile);
	file=xmalloc(strlen(prefs_get_charsave_dir())+1+strlen(charname)+1);
	d2char_get_savefile_name(file,charname);
	rename(file,bakfile);
	...
	eventlog(eventlog_level_trace,__FUNCTION__,"character %s(*%s) deleted",charname,account);
	return 0;
}
