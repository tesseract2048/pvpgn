...
DECLARE_PACKET_HANDLER(on_d2gs_getconf)
...
/* 0x15 */ { sizeof(t_d2gs_d2cs_getconf),        conn_state_authed,     on_d2gs_getconf         },
...
static void d2gs_send_server_conffile(t_d2gs *gs, t_connection *c)
{
	...
	rpacket = packet_create(packet_class_d2gs);
...
static int on_d2gs_updategameinfo(t_connection * c, t_packet * packet)
{
	...
	if (flag==D2GS_D2CS_UPDATEGAMEINFO_FLAG_ENTER) {
		game_add_character(game,charname,charclass,charlevel,ntohl(bn_int_get(packet->u.d2gs_d2cs_updategameinfo.addr)));
...
static int on_d2gs_getconf(t_connection * c, t_packet * packet)
{
	t_d2gs		* gs;
	if (!(gs=d2gslist_find_gs(conn_get_d2gs_id(c)))) {
		eventlog(eventlog_level_error,__FUNCTION__,"game server %d not found",conn_get_d2gs_id(c));
