...
static t_list		* gamelisthp_head=NULL;
static t_elem const	* gamelisthp_curr_elem=NULL;

static unsigned int	total_game=0;
static unsigned int	total_player=0;
...
extern int d2cs_gamelist_create(void)
{
	...
	gamelisthp_head=list_create();
	return 0;
}


extern t_list * d2cs_gamelisthp(void)
{
	return gamelisthp_head;
}

extern t_elem const * gamelisthp_get_curr_elem(void)
{
	return gamelisthp_curr_elem;
}

extern void gamelisthp_set_curr_elem(t_elem const * elem)
{
	gamelisthp_curr_elem=elem;
	return;
}
...
extern int d2cs_gamelist_destroy(void)
{
	...
	list_destroy(gamelisthp_head);
	gamelist_head=NULL;
	gamelisthp_head=NULL;
	return 0;
}
...
{
	char lowername[MAX_GAMENAME_LEN+1];
	t_game	* game;
	time_t	now;
	int i, len;
	...
	//TODO: check if there is a special name
	strcpy(lowername, gamename);
	len = strlen(lowername);
	for (i = 0; i < len; i ++)
	{
		if (lowername[i] >= 'A' && lowername[i] <= 'Z') lowername[i] += 32;
	}
	if (strstr(lowername, "kb") != NULL || strstr(lowername, "qh") != NULL || strstr(lowername, "kc") != NULL || strstr(lowername, "up") != NULL || strstr(lowername, "kd") != NULL || strstr(lowername, "pass") != NULL)
	{
		game->high_priority = 1;
		list_prepend_data(gamelisthp_head,game);
	}
	else
	{
		game->high_priority = 0;
	}
	total_game++;
...
extern int game_destroy(t_game * game, t_elem ** elem)
{
	...
	if (game->high_priority == 1)
	{
		if (gamelisthp_curr_elem && (game==elem_get_data(gamelisthp_curr_elem))) {
			gamelisthp_curr_elem=elem_get_next_const(gamelisthp_head,gamelisthp_curr_elem);
		}
		if (list_remove_data(gamelisthp_head,game,elem)<0) {
			eventlog(eventlog_level_error,__FUNCTION__,"error remove game %s on high priority game list",game->name);
		}
	}
	total_game--;
...
extern t_game_charinfo * game_addr_num(t_game * game, int addr)
{
	t_game_charinfo * charinfo;
	int count = 0;

	//eventlog(eventlog_level_info,__FUNCTION__,"game_addr_num");
	ASSERT(game,NULL);
	if (!game->charlist) {
		return NULL;
	}
	...
	{
		if (!charinfo->charname) continue;
		if (charinfo->addr == addr) count ++;
	}
	END_LIST_TRAVERSE_DATA()

	//eventlog(eventlog_level_info,__FUNCTION__,"addr: %x, user: %d", addr, count);
	return count;
}

extern int game_add_character(t_game * game, char const * charname, unsigned char class,
				unsigned char level, int addr)
{
	t_game_charinfo	* charinfo;
	...
	eventlog(eventlog_level_info,__FUNCTION__,"added character %s (addr: %x) to game %s (%d total)",charname, addr, game->name,game->currchar);
	total_player++;
	return 0;
}
...
extern unsigned int gamelist_get_totalplayer(void)
{
	return total_player;
}
