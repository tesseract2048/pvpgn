...
static int on_client_loginreq(t_connection * c, t_packet * packet)
{
	...
	if (!bnetd_conn()) {
		eventlog(eventlog_level_fatal,__FUNCTION__,"d2cs is offline with bnetd, login request will be rejected");
		return -1;
	}
...
static int on_client_createcharreq(t_connection * c, t_packet * packet)
{
	...
	if (d2char_create(account,charname,class,status)<0) {
		...
		if ((bnpacket=packet_create(packet_class_d2cs_bnetd))) {
			if ((sq=sq_create(d2cs_conn_get_sessionnum(c),packet,0))) {
				packet_set_size(bnpacket,sizeof(t_d2cs_bnetd_charoptsignal));
				packet_set_type(bnpacket,D2CS_BNETD_CHAROPTSIGNAL);
				bn_int_set(&bnpacket->u.d2cs_bnetd_charoptsignal.h.seqno,sq_get_seqno(sq));
				bn_int_set(&bnpacket->u.d2cs_bnetd_charoptsignal.opt_type,3);
				packet_append_string(bnpacket,account);
				packet_append_string(bnpacket,charname);
				packet_append_string(bnpacket,inet_ntoa(addr));
				conn_push_outqueue(bnetd_conn(),bnpacket);
			}
			packet_del_ref(bnpacket);
		}
...
static int on_client_creategamereq(t_connection * c, t_packet * packet)
{
	...
	if(d2cs_conn_get_vip_expire(c) < time(NULL)){
		gamepass[0] = 0;
	}
	...
	if (d2cs_gamelist_find_game(gamename)) {
		...
		if (gq) {
			...
			gq_destroy(gq,&elem);
			return 0;
		} else if ((gq=gq_create(d2cs_conn_get_sessionnum(c), packet, gamename, (d2cs_conn_get_vip_expire(c) >= time(NULL))))) {
	...
	if (reply!=D2CS_CLIENT_CREATEGAMEREPLY_SUCCEED) {
		...
		if (gs->active && gs->connection && (gspacket=packet_create(packet_class_d2gs))) {
...
static int on_client_joingamereq(t_connection * c, t_packet * packet)
{
	...
	} else if (!(gs=game_get_d2gs(game)) || !gs->active) {
	...
	if (reply!=D2CS_CLIENT_JOINGAMEREPLY_SUCCEED) {
		...
		if (gs->active && gs->connection && (gspacket=packet_create(packet_class_d2gs))) {
			if ((sq=sq_create(d2cs_conn_get_sessionnum(c),packet,d2cs_game_get_id(game)))) {
				...
				bn_int_set(&gspacket->u.d2cs_d2gs_joingamereq.vip_expire,d2cs_conn_get_vip_expire(c));
			...
			if ((bnpacket=packet_create(packet_class_d2cs_bnetd))) {
				if ((sq=sq_create(d2cs_conn_get_sessionnum(c),packet,0))) {
					sprintf(buf, "%s@%s", gamename, prefs_get_realmname());
					packet_set_size(bnpacket,sizeof(t_d2cs_bnetd_joingamesignal));
					packet_set_type(bnpacket,D2CS_BNETD_JOINGAMESIGNAL);
					bn_int_set(&bnpacket->u.d2cs_bnetd_joingamesignal.h.seqno,sq_get_seqno(sq));
					packet_append_string(bnpacket,account);
					packet_append_string(bnpacket,charname);
					packet_append_string(bnpacket,inet_ntoa(addr));
					packet_append_string(bnpacket,buf);
					packet_append_string(bnpacket,gamepass);
					conn_push_outqueue(bnetd_conn(),bnpacket);
				}
				packet_del_ref(bnpacket);
			}
			/*
			if(gamepass[0]=='\0'){
				eventlog(eventlog_level_trace,__FUNCTION__,"account %s char %s from %s join game %s",account,charname,inet_ntoa(addr),
					gamename);
			}else{
				eventlog(eventlog_level_trace,__FUNCTION__,"account %s char %s from %s join game %s with password %s",account,charname,inet_ntoa(addr),
					gamename,gamepass);
			}*/
		}
	}
...
static int on_client_gamelistreq(t_connection * c, t_packet * packet)
{
	...
	//gamelisthp
	elem=start_elem=gamelisthp_get_curr_elem();
	if (!elem) elem=list_get_first_const(d2cs_gamelisthp());
	else elem=elem_get_next_const(d2cs_gamelisthp(),elem);

	for (; elem != start_elem; elem=elem_get_next_const(d2cs_gamelisthp(),elem)) {
		if (!elem) {
			elem=list_get_first_const(d2cs_gamelisthp());
		...
		if ((rpacket=packet_create(packet_class_d2cs))) {
			...
			packet_del_ref(rpacket);
			count++;
			if (count > 6 || (prefs_get_maxgamelist() && count>=prefs_get_maxgamelist())) break;
		}
	}
	gamelisthp_set_curr_elem(elem);

	//gamelist
	if (prefs_get_maxgamelist() && count>=prefs_get_maxgamelist())
	{
		//enough
	}
	else
...
static int on_client_charloginreq(t_connection * c, t_packet * packet)
{
	...
	if ((d2cs_conn_get_vip_expire(c) + expire_time) < time(NULL) && (vip_realm || (expire_time && (time(NULL) > bn_int_get(data.header.last_time) + expire_time)))) {
		t_packet * rpacket;
...
static int on_client_deletecharreq(t_connection * c, t_packet * packet)
{
	...
	if (d2char_delete(account,charname)<0) {
		...
		t_sq		* sq;
		addr.s_addr = htonl(d2cs_conn_get_addr(c));
		//eventlog(eventlog_level_trace,__FUNCTION__,"char %s (*%s) deleted from %s",charname,account,inet_ntoa(addr));
		...
		if ((bnpacket=packet_create(packet_class_d2cs_bnetd))) {
			if ((sq=sq_create(d2cs_conn_get_sessionnum(c),packet,0))) {
				...
				packet_set_type(bnpacket,D2CS_BNETD_CHAROPTSIGNAL);
				bn_int_set(&bnpacket->u.d2cs_bnetd_charoptsignal.h.seqno,sq_get_seqno(sq));
				bn_int_set(&bnpacket->u.d2cs_bnetd_charoptsignal.opt_type,1);
				packet_append_string(bnpacket,account);
				packet_append_string(bnpacket,charname);
				packet_append_string(bnpacket,inet_ntoa(addr));
				conn_push_outqueue(bnetd_conn(),bnpacket);
...
static int on_client_charlistreq(t_connection * c, t_packet * packet)
{
	...
	d2char_get_infodir_name(path,account);
	maxchar=d2cs_conn_get_vip_expire(c) >= time(NULL) ? MAX_MAX_CHAR_PER_ACCT : prefs_get_maxchar();
	if ((rpacket=packet_create(packet_class_d2cs))) {
	...
	if ((rpacket=packet_create(packet_class_d2cs))) {
		...
		if (!(dir=p_opendir(path))) {
			...
			while ((charname=p_readdir(dir))) {
				...
				if (exp_time && d2cs_conn_get_vip_expire(c) < time(NULL)) {
					if (d2cs_conn_get_vip_expire(c) + exp_time < bn_int_get(charinfo->header.last_time)+exp_time)
					{
						curr_exp_time = bn_int_get(charinfo->header.last_time)+exp_time;
					}
					else
					{
						curr_exp_time = d2cs_conn_get_vip_expire(c) + exp_time;
					}
				} else {
	...
	if (d2char_convert(account,charname)<0) {
		...
		if ((bnpacket=packet_create(packet_class_d2cs_bnetd))) {
			if ((sq=sq_create(d2cs_conn_get_sessionnum(c),packet,0))) {
				...
				packet_set_type(bnpacket,D2CS_BNETD_CHAROPTSIGNAL);
				bn_int_set(&bnpacket->u.d2cs_bnetd_charoptsignal.h.seqno,sq_get_seqno(sq));
				bn_int_set(&bnpacket->u.d2cs_bnetd_charoptsignal.opt_type,2);
				packet_append_string(bnpacket,account);
				packet_append_string(bnpacket,charname);
				packet_append_string(bnpacket,inet_ntoa(addr));
				conn_push_outqueue(bnetd_conn(),bnpacket);
...
static int d2cs_check_joingame_limit(t_connection const * c, t_game const * game)
{
	int t = time(NULL);
	return 0;
	t = t % 86400 / 3600;
	if (t > 15) return 0;
	if (game_get_gameflag_expansion(game) != 0) return 0;
	//d2cs_conn_get_vip_expire(c) < time(NULL) && 
	if (conn_get_charinfo_level(c) > 20) return 0;
	if (game_addr_num(game, d2cs_conn_get_addr(c)) >= 3)
	{
		return -1;
	}
	return 0;
...
static unsigned int d2cs_try_joingame(t_connection const * c, t_game const * game, char const * gamepass)
{
	...
	if (!game_get_created(game)) {
		...
		if (game_get_maxchar(game) <= game_get_currchar(game) || d2cs_check_joingame_limit(c, game) != 0) {
		...
		if (game_get_currchar(game) >= MAX_CHAR_PER_GAME || d2cs_check_joingame_limit(c, game) != 0) {
			reply=D2CS_CLIENT_JOINGAMEREPLY_GAME_FULL;
		} else {
			reply=D2CS_CLIENT_JOINGAMEREPLY_SUCCEED;
		}
